# AI Reference — OPC UA Test Server Suite
...
```
PORT  SERVICE               POLICIES                                          MODES                 ANON  USER  CERT  AUTO-ACCEPT
4840  opcua-no-security     None                                              None                  yes   no    no    no
4841  opcua-userpass        Basic256Sha256                                    SignAndEncrypt         no    yes   no    no
4842  opcua-certificate     Basic256Sha256,Aes128_Sha256_RsaOaep,Aes256_Sha256_RsaPss  Sign,SignAndEncrypt  no    no    yes   no
4843  opcua-all-security    None,Basic128Rsa15,Basic256,Basic256Sha256,Aes128_Sha256_RsaOaep,Aes256_Sha256_RsaPss  None,Sign,SignAndEncrypt  yes  yes  yes  no
4844  opcua-discovery       None,Basic256Sha256                               None,SignAndEncrypt    yes   no    no    no
4845  opcua-auto-accept     Basic256Sha256                                    SignAndEncrypt         yes   yes   yes   yes
4846  opcua-sign-only       Basic256Sha256                                    Sign                   yes   yes   no    no
4847  opcua-legacy-security Basic128Rsa15,Basic256                            Sign,SignAndEncrypt    yes   yes   no    no
```
...
Server SAN: `localhost`, all container names, `127.0.0.1`, `0.0.0.0`, `urn:opcua:test-server:OPCUATestServer`

## Namespace

```
ns=0  OPC UA standard namespace
ns=1  Test server namespace
      URI: urn:opcua:test-server:<ServerName>
      NodeIds: auto-generated numeric (ns=1;i=XXXX)
      Use BrowsePaths for stable references
```
...
```
PATH                                         TYPE    DIMENSIONS  STORAGE          INITIAL
DataTypes/MultiDimensional/Matrix2D_Double   Double  [3,3]       Float64Array     [1,2,3,4,5,6,7,8,9]
DataTypes/MultiDimensional/Matrix2D_Int32    Int32   [2,4]       Int32Array       [1,2,3,4,5,6,7,8]
DataTypes/MultiDimensional/Cube3D_Byte       Byte    [2,3,4]     Uint8Array       [0..23]
```

valueRank=2 for 2D, valueRank=3 for 3D. Data is flat row-major. VariantArrayType=Matrix.
...
Methods/GetServerTime   (none)                                          time:DateTime                                   new Date()
...
Methods/LongRunning     durationMs:UInt32                               completed:Boolean                               setTimeout(min(dur,30000)), returns true
Methods/Failing         (none)                                          (none)                                          always BadInternalError
Methods/ArraySum        values:Double[] (valueRank=1)                   sum:Double                                      reduce(+)
Methods/MatrixTranspose matrix:Double[],rows:UInt32,cols:UInt32         result:Double[] (VariantArrayType.Array)         transpose; BadInvalidArgument if len!=rows*cols
...
Dynamic/Random           Double      500ms timer  Math.random() → [0,1)
Dynamic/RandomInt        Int32       1s timer     floor(random()*2001)-1000 → [-1000,1000]
...
Dynamic/Timestamp        DateTime    on-read      new Date()
...
Dynamic/NullableDouble   Double      4s timer     alternates Good(random 0-100) / BadNoData; uses timestamped_get
```
...
```
PERIODIC EVENTS:
SimpleEventType          every 5s    severity=200    message="Periodic event #N"    payload="payload-N"
SystemStatusEventType    every 15s   severity=100/600  states=[Running,Idle,Busy,Maintenance]  cpu=random  mem=40+random*50
```
...
Historical/HistoricalTemperature    Double   1s    10000        22+8*sin(t/60)+(random-0.5)*2
Historical/HistoricalPressure       Double   1s    10000        101.325+5*cos(t/120)+(random-0.5)
Historical/HistoricalCounter        UInt32   1s    10000        ++counter
Historical/HistoricalBoolean        Boolean  1s    10000        random()>0.5
```

accessLevel = CurrentRead | HistoryRead. In-memory storage, not persisted across restarts.
...
ALL COMBINATIONS (32 vars = 8 types × 4 access modes):
...
```
src/
├── index.js                     main(): creates OPCUAServer or OPCUADiscoveryServer, prints endpoints, graceful shutdown
├── config.js                    parseList/parseBool/parseIntEnv from env vars → config object
├── user-manager.js              loadUsers() from config/users.json, isValidUser(), getUserRoles()
└── address-space/
    ├── index.js                 constructAddressSpace(): creates TestServer folder, calls modules based on config toggles
    ├── data-types.js            buildDataTypes(): buildScalars + buildReadOnlyScalars + buildArrays + buildMultiDimensionalArrays + buildRangeVariables
    ├── methods.js               buildMethods(): 12 method definitions with bindMethod callbacks
    ├── dynamic.js               buildDynamic() + stopDynamic(): timer-based and on-read variables
...
namespace.addVariable({
  ...
  value: {
    get: () => new Variant({ dataType: DataType.XXX, value: value }),
    set: (variant) => { value = variant.value; return StatusCodes.Good; },
  },
});
```
...
const method = namespace.addMethod(parentFolder, {
  browseName: "Name",
  inputArguments: [{ name: "arg", dataType: DataType.XXX, description: { text: "desc" } }],
  outputArguments: [{ name: "result", dataType: DataType.XXX, description: { text: "desc" } }],
});
method.bindMethod((inputArguments, context, callback) => {
  callback(null, {
    statusCode: StatusCodes.Good,
    outputArguments: [{ dataType: DataType.XXX, value: result }],
  });
});
```
...
```js
const timers = [];
timers.push(setInterval(() => { /* update value */ }, intervalMs));
function stop() { for (const t of timers) clearInterval(t); timers.length = 0; }
```

Stop functions are called in graceful shutdown (SIGINT/SIGTERM) from index.js.

### Module pattern

Each address-space module exports `build*()` and optionally `stop*()`. The orchestrator (`address-space/index.js`) calls build functions conditionally based on `config.enable*` flags.

## Environment Variables (Complete)

```
OPCUA_PORT                    int     4840        Server port
OPCUA_SERVER_NAME             string  OPCUATestServer  Display name, used in namespace URI
...
OPCUA_USERS_FILE              path    config/users.json  User credentials file
OPCUA_CERTIFICATE_FILE        path    certs/server/cert.pem
OPCUA_PRIVATE_KEY_FILE        path    certs/server/key.pem
OPCUA_TRUSTED_CERTS_DIR       path    certs/trusted
OPCUA_REJECTED_CERTS_DIR      path    certs/rejected
OPCUA_CA_CERT_FILE            path    certs/ca/ca-cert.pem
OPCUA_AUTO_ACCEPT_CERTS       bool    false       Auto-accept unknown client certs
OPCUA_MAX_SESSIONS            int     100
OPCUA_MAX_SUBSCRIPTIONS       int     100
OPCUA_MIN_PUBLISHING_INTERVAL int     100         Milliseconds
OPCUA_DISCOVERY_URL           string  ""          Discovery server URL for registration
OPCUA_IS_DISCOVERY            bool    false       Run as OPCUADiscoveryServer
...
Write as viewer role                        BadUserAccessDenied
...
docker compose down -v        # stop + delete cert volume
